...
export default function FeaturesBar() {
  ...
  // Obtener las primeras 3 características para mostrar
  const features = home.features.items.slice(0, 3).map((item, index) => {
    // Resolver el icono dinámicamente
    const IconComponent = OutlineIcons[item.iconName] || OutlineIcons.StarIcon

    // Asignar colores rotativos o específicos
    let color = colors.primary.hex
    if (index === 1) color = colors.secondary.hex
    if (index === 2) color = colors.primary.dark
  ...
  return (
    <section
      ...
      style={{
        backgroundColor: theme.navbar?.backgroundColor || '#0f172a'
      }}
    >
